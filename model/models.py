...
class StTotalView(Base):
    ...
    @staticmethod
    def select(target_startdate, target_enddate, session):
        return session.query(StTotalView).filter(StTotalView.date.between(target_startdate, target_enddate)).all()

    @staticmethod
    def count(target_date, session):
        return session.query(StTotalView).filter(StTotalView.date == target_date).count()

    @staticmethod
    def regist(target_date, target_amount, session):
        session.add(StTotalView(date=target_date, amount=target_amount))

    @staticmethod
    def update(target_date, target_amount, session):
        session.add(StTotalView(date=target_date, amount=target_amount))
...
class Stream(Base):
    ...
    @staticmethod
    def count(target_id, session):
        return session.query(Stream).filter(Stream.id == target_id).count()

    @staticmethod
    def regist(target_id, target_title, target_author, target_startdate, session):
        session.add(Stream(id=target_id, title=target_title, author=target_author, startdate=target_startdate))

    @staticmethod
    def update(target_id, target_title, target_author, target_startdate, session):
        # session.add(Stream(id=target_id, title=target_title, author=target_author, startdate=target_startdate))
